...
import {AnumaAPI, ApiError} from './api';
...
const SompiPerAnuma = 100_000_000
...
class Wallet extends EventTargetImpl {
	...
	static networkTypes: Object = {
		anuma: { port: 12412, network: 'anuma-mainnet', name : 'mainnet' },
		anumatest: { port: 12512, network: 'anuma-testnet', name : 'testnet' },
		anumasim: {	port: 12612, network: 'anuma-simnet', name : 'simnet' },
		anumadev: {	port: 12712, network: 'anuma-devnet', name : 'devnet' },
	}
	 ...
	 * Current network.
	 */
	network: Network = 'anuma';

	api: AnumaAPI; //new AnumaAPI();
	...
	constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
		super();
		this.logger = CreateLogger('AnumaWallet');
		this.api = new AnumaAPI();
		//@ts-ignore
	...
	*/
	 ...
	 * @param obj
	 * @param obj.toAddr To address in cashaddr format (e.g. anumatest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
	 * @param obj.amount Amount to send in sompis (100000000 (1e8) sompis in 1 ANUM)
	...
	*/
	...
	async composeTxAndNetworkFeeInfo(txParamsArg: TxSend): Promise < TxInfo >{
		...
		this.logger.info(`tx ... amount: ${ANUM(txParamsArg.amount)} user fee: ${ANUM(txParamsArg.fee)} max data fee: ${ANUM(txParamsArg.networkFeeMax||0)}`)
		...
		this.logger.verbose(`tx ... need data fee: ${ANUM(dataFee)} total needed: ${ANUM(amountRequested+dataFee)}`)
		this.logger.verbose(`tx ... available: ${ANUM(amountAvailable)} in ${data.utxos.length} UTXOs`)

		if(networkFeeMax && dataFee > networkFeeMax) {
			throw new Error(`Fee max is ${networkFeeMax} but the minimum fee required for this transaction is ${ANUM(dataFee)} ANUM`);
		}
		...
		if(calculateNetworkFee){
			do {
				...
				this.logger.verbose(`tx ... need data fee: ${ANUM(dataFee)} for ${data.utxos.length} UTXOs`);
			...
			if(networkFeeMax && txParams.fee > networkFeeMax)
				throw new Error(`Maximum network fee exceeded; need: ${ANUM(dataFee)} ANUM maximum is: ${ANUM(networkFeeMax)} ANUM`);

		}else if(dataFee > priorityFee){
			throw new Error(`Minimum fee required for this transaction is ${ANUM(dataFee)} ANUM`);
	...
	async buildTransaction(txParamsArg: TxSend, debug = false): Promise < BuildTxResult > {
		...
		this.logger.info(`tx ... required data fee: ${ANUM(dataFee)} (${utxos.length} UTXOs)`);// (${ANUM(txParamsArg.fee)}+${ANUM(dataFee)})`);
		//this.logger.verbose(`tx ... final fee: ${ANUM(dataFee+txParamsArg.fee)} (${ANUM(txParamsArg.fee)}+${ANUM(dataFee)})`);
		this.logger.info(`tx ... resulting total: ${ANUM(totalAmount)}`);
