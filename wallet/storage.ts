...
class LSDB extends DBInterface{
	...
	backup():void{
		...
		if(!data)
			return
		let ts = Date.now();
		this.LS.setItem("anuma-wallet-"+ts, data);
		let cache = this.getCache();
		if(cache)
			this.LS.setItem("anuma-cache-"+ts, cache);
	}
	saveWallet(data:string):void{
		this.LS.setItem("anuma-wallet", data);
	}

	getWallet():string|false{
		return this.LS.getItem("anuma-wallet");
	}

	saveCache(cache:string){
		return this.LS.setItem("anuma-cache", cache);
	}

	getCache():string|undefined{
		return this.LS.getItem("anuma-cache");
	}
}
...
if(IS_NODE){
	...
	class FileDB extends DBInterface{
		...
		getHomeFolder(){
			...
			if(APPDATA) // windows
				return path.join(APPDATA, 'Anuma');

			if(process.platform == 'darwin')
				return path.join(HOME,'/Library/Application Support/Anuma/');

			return path.join(HOME, '.anuma');
		}
...
export class Storage{
	...
	constructor(opt:StorageOpts={}){
		this.logger = CreateLogger("AnumaStorage");
